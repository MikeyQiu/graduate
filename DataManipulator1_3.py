...
class Neo4jToJson1_3(object):
    ...
    def post(self):
        """与前端交互"""
        links_data = self.graph.run('MATCH p=(source)-[r:Go_Study]->(target) RETURN source.Name as source,target.Name as target UNION MATCH p=(source)-[r:Go_Work]->(target) RETURN source.Name as source,target.Name as target UNION MATCH p=(source)-[r:Located_In]->(target) RETURN source.Name as source,target.Name as target').data()
        nodes_data = self.graph.run("match (n:GoStudy) return n  UNION match (n:School) return n UNION match (n:GoWork) return n UNION match (n:Region) return n").data()
